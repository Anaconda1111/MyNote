...
1.  参数： a -- 压缩  x -- 解压
 	...
 	2. 压缩： zip （压缩包名字、不需要后缀）  压缩的文件或目录
 	3. 解压： unzip （压缩文件名） ，如果需要指定解压到哪个目录则需要参数-d

## 五、 进程相关的命令

 	1. ps：查看当前用户下所有进程。其参数为 -a：all，表示所有进程、 -u：表示显式一些其他信息、 -x：显式没有控制终端的进程。一般的使用为  ps  aux。
 	2. 管道：将命令1的输出作为命令2的输入，例如： ps aux | grep bash。将ps命令输出的进程名输入到grep命令中进行查找。
 	3. kill：kill -l：显示可执行的操作。  kill -SIGKILL + 进程号：可以杀死进程。
 	4. env：查看当前进程的环境变量。Linux下的环境变量是key - value格式，key可以对应多个value，用 : 分隔。

## 六、网络相关命令

1.  ifconfig：显示当前电脑网卡信息、ip地址、MAC地址等。

 	2. ping：连接另一台主机或服务器，后面加ip地址或域名。
 	3. nslookup：后加域名，可显示对方的ip地址。

## 七、用户管理：

### 7.1 添加用户

1.  sudo adduser  +用户名：比较简单的方式来创建一个新用户

 	2. sudo useradd：参数比较多  -s:指定新用户登录时shell类型    -g：指定所属组，必须已存在    -d：用户家目录   -m：用户家目录不存在时自动创建该目录。例： sudo useradd -s /bin/bash  -g itcast  -d /home/itcast  -m itcast
...
1.  sudo deluser +用户名
 	...
 	2. sudo deluser  -r +用户名（可以将主目录一起删除）
...
​	环境变量是指在操作系统中用来指定操作系统运行环境的一些参数，本质上是一系列字符串，形式为key:value，多个value之间用冒号分隔，用来描述**进程**的环境信息（即不同进程的环境变量可能不同）。用代码查看环境变量的方法是，extren char ** environ，然后逐个打印字符串数组environ中的值，直到为NULL。