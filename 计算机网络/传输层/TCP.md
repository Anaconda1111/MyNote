...
​	其次是**序列号**：上述提到的确认应答、超时重传、重复控制等机制都是通过序列号实现的，序列号是**按发送顺序给发送数据的每一个字节都标上号码的编号**，接受端**查询接受数据TCP首部中的序列号和数据长度，将下一步应该接受的序列号作为确认应答返送回去**，例如收到了1~1000号的数据，下一次应该从1001号数据开始，那么确认应答就发送1001。
...
​		TCP的keep alive机制也成为保活机制，当建立TCP连接后双方长时间内没有数据交互时，超过一定时间后，内核就会控制TCP发送一个探测报文，来确保连接状态正常。如果对端回复ACK，就重置保活计时器；如果对端系统崩溃，无法回应响应报文，则探测的那一方会重新发送几次探测报文，当达到一定次数后就认为连接异常，内核将错误信息报告给上层应用程序。
​	注：要使用保活机制，必须在建立socket时设置SO_KEEPALIVE

### 9.1 相关参数

  	1.  net.ipv4.tcp_keepalive_time=7200;   表示保活定时器为7200秒，即如果7200秒内没有数据往来就触发保活机制，发送探测报文。
  	2.  net.ipv4.tcp_keepalive_intvl=75;   表示每个探测报文间隔75秒
  	3.  net.ipv4.tcp_keepalive_probes=9; 表示最多发送9次探测报文，如果9次都没有回应则认为连接异常

## 一些问题

### 1. TCP三次握手中，这三个报文发生丢失时，会有什么情况

1.  第一次握手报文丢失：连接主动发起方收不到ACK+SYN报文，一定时间后触发超时重传，重新发送第一次握手报文
2.  第二次握手报文丢失：第二次握手报文是服务端返回给客户端的，**即是ACK报文，又是携带自身序列号的SYN报文**，第二次握手丢失，服务端方就收不到第三次握手的ACK报文，会触发超时重传机制；而客户端收不到第二次握手报文，也会超时重传第一次握手报文。结论就是：两方都会超时重传。
3.  第三次握手报文丢失：第三次是单纯的ACK报文，丢失时被动连接方超时重传第二次握手。
